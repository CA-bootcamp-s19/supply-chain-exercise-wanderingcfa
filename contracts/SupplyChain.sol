...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
  ...
  modifier contract_owner (address _address) { require (msg.sender == owner); _;}
  ...
  modifier forSale (uint _sku) {require (items[_sku].state == 0 && items[_sku].price >= 0); _;}
  modifier sold (uint _sku) { require (items[_sku].state == 1 && items[_sku].buyer != address(0)); _;}
  modifier shipped (uint _sku) { require (items[_sku].state == 2 && items[_sku].buyer != address(0)); _;}
  modifier received (uint _sku) { require (items[_sku].state == 3 && items[_sku].buyer != address(0)); _;}
  ...
  function addItem(string memory _name, uint _price) public returns(bool){
    items[skuCount] = Item({name: _name, sku: skuCount, price: _price, state: 0, seller: msg.sender, buyer: address(0)});
